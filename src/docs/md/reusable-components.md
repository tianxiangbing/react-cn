...
一个通用的场景是：一个组件需要定期更新。用 `setInterval()` 做很容易，但当不需要它的时候取消定时器来节省内存是非常重要的。React 提供 [生命周期方法](/docs/working-with-the-browser.html#component-lifecycle) 来告知你组件创建或销毁的时间。下面来做一个简单的 mixin，使用 `setInterval()` 并保证在组件销毁时清理定时器。
...
这对应用的性能有帮助，特别是当你用 [浅层比较](/docs/shallow-compare.html) 实现 [shouldComponentUpdate()](/docs/component-specs.html#updating-shouldcomponentupdate) 时。
